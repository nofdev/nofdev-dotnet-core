...
namespace Nofdev.Server.Controllers
{
    ...
    public  class RpcApiController : ControllerBase
    {
            ...
            string methodName,  string @params)
        {
            ...
            try
            {
                var serviceLayer = HttpContext.GetServiceLayer();
                _logger.LogDebug($"ask instance for interface {interfaceName}");

                var serviceType = ServiceBootstrapper.GetServiceType(packageName, interfaceName, serviceLayer);
